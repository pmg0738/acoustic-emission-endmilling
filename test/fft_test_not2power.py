...
# 関数の集まり
def FFT_main2(t, x, dt, split_t_r, overlap, window_F):

    # データをオーバーラップして分割する。
    split_data = data_split2(t, x, split_t_r, overlap)

    # FFTを行う。
    FFT_result_list = []
    for split_data_cont in split_data:
        FFT_result_cont = FFT2(split_data_cont, dt, window_F)
        FFT_result_list.append(FFT_result_cont)

    # 平均化
    fq_ave = FFT_result_list[0][0]
    F_abs_amp_ave = np.zeros(len(fq_ave))
    print(f"fq_ave{len(fq_ave)}")
...
def FFT2(data_input, dt, window_F):
...
def data_split2(t, x, split_t_r, overlap):
...
def plot_FFT2(fq, F_abs_amp, output_FN, y_label, y_unit, num):
